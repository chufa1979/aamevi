...
AAMEVI is an e-learning platform built with Laravel 12 + Blade + MySQL 8. The business modules are built: enrollment, quiz, progression, tasks, certificates, notifications, the admin/teacher Filament panels, and a public course showcase — see "Known gaps" below for what's still missing.

The authoritative spec for what gets built is `docs/PLAN_ARQUITECTONICO.md` (Spanish) — full normalized SQL schema, per-module component breakdown, and the main user flows. It's a living document: sections marked **"Desviación implementada"** record where the shipped schema diverges from the original plan. **Read the relevant section of that doc before implementing a feature**; the entity definitions, enum values, and unique constraints there are the contract. `README.md` summarizes the same flows at a higher level, and `docs/DEPLOY.md` covers the target hosting.
...
**App** — Laravel 12 (PHP 8.2+) + Blade + Eloquent ORM + MySQL 8 (InnoDB, `utf8mb4_unicode_ci`). `composer.json` pins `config.platform.php` to **8.4.0**, the floor of what the deployment target's CLI offers — dependencies must resolve against that, not against whatever runs locally. The pin moved here from 8.3.11 when the deployment target changed: the original host's CLI topped out at 8.3, `aamevicampus.com.ar`'s only offers 8.2 and 8.4 (see `docs/DEPLOYMENT_GUIDE.md`). That incidentally clears the version floor Symfony 8 needs, but staying on Laravel 12 is still the current choice — upgrading to 13 is a separate decision, not implied by this pin. Business logic lives in feature modules under `app/Http/Controllers/` organized by domain (`Auth`, `Users`, `Courses`, `Quiz`, `Tasks`, `Notifications`, `Storage`, `Certificates`, `Reports`), with models in `app/Models/`, form requests for validation in `app/Http/Requests/`, and middleware in `app/Http/Middleware/`. Migrations live in `database/migrations/`, seeders in `database/seeders/`.
...
**Domain model** (see `docs/PLAN_ARQUITECTONICO.md` §2 for full DDL). All PKs are UUIDs. `users` is the auth base table; `students` and `teachers` are 1:1 extensions keyed on `users.id`. Content hierarchy is `courses → modules → classes → class_content`. Enrollment (`course_enrollments`) is a state machine: `pending → approved/rejected → active → completed`, approved by an admin or the `Registrar` role — not the teacher, who only sees the roster now (see "Two surfaces" below). `approved_by` references `users`, not `teachers`, because none of the roles that can resolve a request necessarily have a teacher record.

`courses` carries two kinds of columns: operational ones that gate enrollment (`teacher_id`, `max_students`, `is_active`), and showcase ones that exist only to render `/curso/{course}` (`start_date`/`end_date`, `modality` — a `CourseModality` enum — `location`, `specialties`, `schedule_days`, `schedule_time`, `investment_info`, `certification_info`, `teaching_staff`, `objectives`, `enrollment_requirements`). All showcase fields are nullable free text, edited from a separate "Ficha pública" section in `CourseForm` — a course works fine with none of them filled in, the public page just shows fewer sections. `start_date`/`end_date` are the one showcase field with a computed value in `CourseSeeder`: they're derived from the same per-class schedule that spaces out `activation_date`, not entered separately, so there's one source of truth for "when does this course run."
...
Filament serves that surface through **three panels over one set of resources**: `/admin` for administrators, `/profesores` for teachers, and `/administracion` for the `Registrar` role — enrollment/registration staff who accept students into courses but don't teach or administer content (`TeacherPanelProvider` and `RegistrarPanelProvider` each register a short, explicit list of the same resource classes — no user accounts, no course content for the registrar). Never scope anything by panel id: what a teacher may see comes from `Course::scopeVisibleTo(User)`, which the resources apply via `App\Filament\Concerns\ScopedToOwnCourses`, and what they may do comes from the policies in `app/Policies/` (`CoursePolicy`, and `CoursePartPolicy` for everything hanging off a course). Both hold whichever URL the request came in through, and because Filament resolves records through the resource query, a teacher who types another teacher's course URL gets a 404.

**Enrollment approval moved out of the teacher's hands, on purpose.** The teacher still sees the roster (`ManageCourseStudents`, the "Alumnos" tab nested under a course) but can't create, approve or reject a request there anymore — those record actions are `->visible(fn (): bool => ! auth()->user()->isTeacher())`. The `Registrar` role gets a global, non-nested page instead (`EnrollmentRequestResource` → `ManageEnrollmentRequests`), listing every course's pending requests in one filterable table, because someone whose whole job is processing enrollments shouldn't have to open each course to find what's pending. Admins keep both: the nested tab and, since `EnrollmentRequestResource` is auto-discovered like every other admin resource, the global one too.

Watch out if you touch `CourseEnrollmentPolicy`: Filament's `ManageRelatedRecords` (what `ManageCourseStudents` extends) authorizes itself against `viewAny` on the **relationship's** model — `CourseEnrollment`, not `Course` — not against `CoursePolicy`. `viewAny`/`view` there include the teacher for exactly that reason; only `create`/`update` are admin-or-registrar. Narrowing `viewAny` to match `create` breaks the *existing* nested tab with a 403 on page load, not just the buttons — this is what happened when the policy was first added, and the fix was splitting the abilities, not scoping the panel.

Almost everything is behind `auth`: `routes/web.php` splits into a public pair of routes (`/` and `curso/{course}`), a `guest` group (login and registration), and an `auth` group (everything else — the classroom, both panels). There is deliberately **no second login form** — the Filament panel does not expose `/admin/login`; admins sign in at `/login` like everyone else, so there is one rate-limited entry point to audit. Where each role lands afterwards comes from `User::homeUrl()` — students to `/mis-cursos`, teachers and admins to their panel. It is used both after signing in and by the `guest` middleware, so opening `/login` with a session already open takes you to your own screen rather than the front page. The URL the auth middleware stashed before asking for credentials still wins — but only if `User::canReach()` says that surface is theirs, because the stash survives in the browser session and belongs to whoever browsed last, not to whoever logged in.

**The public pair is the one deliberate exception**, served by `App\Http\Controllers\Public\` — a showcase, not a second application:

- `/` (`HomeController`) branches on whether there's a session: logged out, it's the catalogue of active courses (`public.home`); logged in, it's the same role-based dashboard as before (`home.blade.php`). Same URL, two views, decided server-side.
- `curso/{course}` (`CourseShowcaseController`) is the public course page — singular, deliberately distinct from the plural `cursos/{course}` (`classroom.course`), which is the private syllabus a student sees only once enrolled. A course with `is_active = false` 404s here even with a direct link — nothing unpublished gets a public URL.
- Both views extend `layouts.app` (the same header/footer as the classroom) and `layouts.guest` (login, register), and both render `<x-login-widget>` in `<x-top-bar>` when there's no session — a dropdown login form (`resources/js/login-widget.js`), not a redirect to `/login`, so anyone reading a course page doesn't lose their place.
- Enrolling from that widget has to survive the trip through `/login`: it passes the current URL as `next`, and `AuthenticatedSessionController::store()` checks it (same-origin only, same `canReach()` guard as `url.intended`) after the session's own intended-URL. `next` covers what `url.intended` can't — pages the `auth` middleware never touched because they're public.
...
- **Intentos shows one row per student and evaluation**, not one per attempt — the flat log buried the only thing worth seeing, which is who got stuck. The row is the latest real attempt so actions have a record to act on; the summary columns come from correlated subqueries, never a query per row.
- **A course opens into tabs** via `getRecordSubNavigation()` with `SubNavigationPosition::Top`: Info general · Planificación · Contenidos · Exámenes · Intentos · Calificaciones · Alumnos · Seguimiento · Comunicación · Consultas. Modules and classes have their own tabs one level down. Ten do not fit on one line, so `admin.css` makes the bar wrap instead of scroll — a horizontal scrollbar is one of the few things people never find. Keep new labels short for the same reason.
...
- **Filament resolves closure arguments by name, not position.** A filter written `->query(fn (Builder $q, array $data) => …)` silently receives nothing and filters nothing — no error, no failing page, just wrong results. Name them `$query` and `$data`. Two filters shipped broken this way before a test caught it.
...
- **Running out of attempts is recoverable, and recovering it is recorded.** A student who exhausts a self-assessment without passing cannot advance — progression requires passing it — and the classroom told them to talk to their teacher while the teacher had nothing to press. `quiz_attempt_resets` opens a new cycle: `attemptsLeft()` counts attempts *since the last reset*, the earlier ones stay in the history, and who unblocked whom is on the record. `attempt_number` keeps counting all of them — there is a `unique(quiz_id, student_id, attempt_number)` and restarting the numbering collides with it.
...
- **The panel counts what is waiting on a person: open tickets.** Loading material or grading happens at the teacher's own pace, but an unanswered question has somebody on the other side — that is the one number the sidebar, the course list and the Consultas tab all show, scoped by `Course::scopeVisibleTo()`. The tab reads its course from the route because Filament asks for badges statically, with no instance.
- **Unread is tracked, and the classroom menu shows the count.** Announcements need a table (`announcement_reads`) because one announcement has many readers; a ticket needs one column (`student_read_at`) because it has one. Both are marked read by *opening the screen* — the board shows every announcement in full and the thread shows every message, so asking for an extra click per item would be inventing work to feed a counter. A count, not a bell: what decides whether to go now is how many.
...
- **Registration is open but gated.** Anyone can create an account at `/registro`; it is always a student, always unverified, and the classroom sits behind `verified`. Enrolment still needs approval from an admin or the `Registrar` role, which is what makes an open sign-up safe. Accounts created from the panel default to verified — the institution is vouching for them.
